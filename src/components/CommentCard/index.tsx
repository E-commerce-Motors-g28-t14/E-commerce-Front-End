...
export const CommentCard = (): JSX.Element => {
    ...
    const mockClients: IMockClients[] = [
        {
            comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industries standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.',
            username: 'Júlia Lima',
    ...
    const relativeDate = (timestamp: string): string => {
        ...
        years > 1 ? commentedAt = `há ${years} anos` : months > 0 && months < 12 ? commentedAt = `há ${months} meses` 
        : days > 0 ? commentedAt = `há ${days} dias`: hours > 0 ? commentedAt = `há ${hours} horas` 
        : minutes > 0 ? commentedAt = `há ${minutes} minutos` 
            ...
            <ul>
                {mockClients.map((client, key) => {
                    return(
                        <li key={key}>
                            <div>
                                <InitialsName fontSize='14' height='32' width='32' name={client.username} />
