...
export const CardAdvertiser = ({user}: { user: IUserResponse }): JSX.Element => {
    ...
    return (
        <ContainerCardAdvertiser>
           ...
           <Link to={`/seller/${user.name}`}> <StyledButton className='big grey1'>{user.name}
                Ver todos anúncios</StyledButton></Link>
        </ContainerCardAdvertiser>
    )
}